    ...
    <div class="card-body d-flex flex-column">
      ...
      <div class="d-flex justify-content-between align-items-start mb-2">
        ...
        <div class="d-flex gap-1" @click.stop>
          ...
          >
            <i :class="userStore.bookmarkedIds.has(program.id) ? 'bi bi-bookmark-fill' : 'bi bi-bookmark'"></i>
          </button>
          <button
            class="btn-bookmark"
...
const countryFlags = {
  ...
  'New Zealand': '🇳🇿', Ireland: '🇮🇪', Japan: '🇯🇵',
  Netherlands: '🇳🇱', Spain: '🇪🇸',
}
const countryFlag = computed(() => countryFlags[props.program.country] || '🌏')

const specTags = computed(() => {
  const tags = props.program.specialization?.split(',').map(t => t.trim()).filter(Boolean) || []
  return tags.slice(0, 3)
})

const formattedTuition = computed(() => {
  return settingsStore.formatMoney(props.program.tuitionFee, props.program.currency)
})
